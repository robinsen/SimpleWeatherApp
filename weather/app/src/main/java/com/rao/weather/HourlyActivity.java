...
import com.rao.weather.databinding.ActivityHourlyBinding;
...
public class HourlyActivity extends AppCompatActivity {
  ...
  private Box<ItemHourly> itemHourlyDBBox;
...
//    itemHourlyDBBox = boxStore.boxFor(ItemHourly.class);
...
//    }
    binding.tempTextView.setText(fiveDayWeather.getDayWeather());
    binding.minTempTextView.setText(fiveDayWeather.getMinTemp());
    binding.maxTempTextView.setText(fiveDayWeather.getMaxTemp());
...
//        .observer(new DataObserver<List<ItemHourly>>() {
//          @Override
//          public void onData(@NonNull List<ItemHourly> data) {
    List<ItemHourly> data = new ArrayList<>();
    List<HashMap<String, String>> hourlist = fiveDayWeather.getTodayHourWeather();
    if (hourlist.size()>0) {
      for (int i=0; i<hourlist.size(); i++) {
         ItemHourly item = new ItemHourly();
         item.setWeather(hourlist.get(i));
         data.add(item);
      }
      if (data.size() > 0) {
         mItemAdapter.clear();
...
//        });
  ...
  private void setChartValues(List<ItemHourly> itemHourlyDBList) {
    List<Entry> entries = new ArrayList<>();
    int i = 0;
    if (AppUtil.isRTL(this)) {
      int j = itemHourlyDBList.size() - 1;
      while (j >= 0) {
        entries.add(new Entry(i, (float) Integer.parseInt(itemHourlyDBList.get(j).getTemp())));
        i++;
        j--;
      }
    } else {
      for (ItemHourly itemHourly : itemHourlyDBList) {
        entries.add(new Entry(i, (float) Integer.parseInt(itemHourly.getTemp())));
        i++;
      }
    }
